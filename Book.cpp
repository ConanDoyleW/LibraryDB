...
void BookDAOImpl::InsertBook(Book book)
{
	...
	sql.Format("insert into pkbook values('%s','%s','%s','%s','%s','%s','%s','%s')",
		book.getName().c_str(), book.getTopic().c_str(), book.getIsbnAndPrice().c_str(),
...
void BookDAOImpl::DeleteBook(string id)
{
	CDaoDatabase* conn = Getconnection();
	CString sql;
	sql.Format("delete from pkbook where isbnprice = '%s'", id.c_str());
	conn->Execute(sql, dbSeeChanges);
	conn->Close();
	delete conn;
}
...
void BookDAOImpl::UpdateBook(string id, string field, string value)
{
	CDaoDatabase* conn = Getconnection();
	CString sql;
	sql.Format("update pkbook set %s = '%s' where isbnprice = '%s'", field.c_str(), 
...
vector<Book> BookDAOImpl::SearchBook(string id)
{
	CDaoDatabase* conn = Getconnection();
